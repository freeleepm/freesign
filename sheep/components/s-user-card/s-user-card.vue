            ...
            " mode="aspectFill" @tap="sheep.$router.go('/pages/user/info')"></image>
				...
				<view>
					<view class="nickname-box ss-flex ss-col-center">
						<view class="nick-name ss-m-r-20">{{ isLogin ? (userInfo.name ? userInfo.name : userInfo.nickname) : '未登录' }}</view>
            <uni-tag v-if="userInfo.authState && isLogin" text="已认证" type="warning" :circle="true"></uni-tag>
            <uni-tag v-if="!userInfo.authState && isLogin" text="未认证" :circle="true" @click="sheep.$router.go('/pages/sign/realname')"></uni-tag>
					</view>
				</view>
			</view>
